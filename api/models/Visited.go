...
func (v *Visited) GetVisiteds(db *gorm.DB, uid uint32) (*[]Visited, error) {
	var err error
	visited := []Visited{}
	err = db.Debug().Model(&Favorites{}).Find(&visited).Error
	if err != nil {
		return &[]Visited{}, err
	}
	return &visited, err
...
func (v *Visited) InitData(placeID string, userID uint32) {
	v.ID = 0
	v.PlaceID = placeID
	v.UserID = userID
...
func (v *Visited) DeleteVisited(db *gorm.DB, place_id string, uid uint32) (int64, error) {
